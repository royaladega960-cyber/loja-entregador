export default async function handler(req, res) {
    ...
    // ==========================================
    // 🔒 CHAVES BLINDADAS (Invisíveis no GitHub)
    // O "process.env" puxa as chaves diretamente do cofre secreto da Vercel
    // ==========================================
    const API_KEY = process.env.PARADISE_API_KEY; 
    const PRODUCT_HASH = process.env.PARADISE_PRODUCT_HASH; 

    // Bloqueia e avisa se você esquecer de configurar lá na Vercel
    if (!API_KEY || !PRODUCT_HASH) {
        console.error("ERRO: Chaves da API não configuradas nas Variáveis de Ambiente da Vercel.");
        return res.status(500).json({ error: 'Erro de configuração do servidor. Chaves ausentes no cofre.' });
    }

    try {
        const { amount, description, customer } = req.body;

        // Monta os dados para a operadora conforme a documentação da Paradise Pags
        const payload = {
            amount: amount,
            ...
            customer: {
                ...
                document: customer.document, // O CPF que vem do frontend
        ...
        if (response.ok && data && data.qr_code) {
            ...
            res.status(400).json({ error: 'Falha ao gerar o PIX. Verifique a documentação.' });
        }
    } catch (error) {
